...
from dataset import Metabook, Metabook_fair
...
def parse_args():
    ...
    parser.add_argument('--num_author', type=int, default=28678, help='num of workers to use')
    parser.add_argument('--num_year', type=int, default=1170, help='num of workers to use')
    parser.add_argument('--num_publisher', type=int, default=5897, help='num of workers to use')
    parser.add_argument('--num_location', type=int, default=58, help='num of workers to use')
    ...
    parser.add_argument('--remove_age', action='store_true', default=False, help='train fair classifier')
    ...
    parser.add_argument('--adv_loss_power',type=float, default=1, help='1: normal, 0: other')
...
def run(args, num_workers=1, log_interval=100, verbose=True, save_path=None):
   ...
   # scheduler = torch.optim.lr_scheduler.StepLR(meta_optimiser, 5000, args.lr_meta_decay)
    ...
    dataloader_valid = DataLoader(Metabook(args, partition='test', test_way='new_user_valid'),#old, new_user, new_item, new_item_user
                                        batch_size=1, num_workers=args.num_workers)
    dataloader_test = DataLoader(Metabook(args, partition='test', test_way='new_user_test'),#old, new_user, new_item, new_item_user
                                        batch_size=1, num_workers=args.num_workers)
    for epoch in range(args.num_epoch):
        ...
        for step, batch in enumerate(dataloader_train):
            ...
            for i in range(args.tasks_per_metaupdate): 
                ...
                output = model(x_qry[i],y_qry[i])
                logits = output[0]
                # group_pred[gender].append(logits)
                for k in range(args.num_grad_steps_inner):
                    output = model(x_spt[i],y_spt[i])
                    logits = output[0]
                    if args.loss==1:
        ...
        for step, batch in enumerate(dataloader_train):
            ...
            for i in range(args.tasks_per_metaupdate): 
                ...
                for l in range(args.num_grad_steps_inner):
                    ...
                    # print(torch.autograd.grad(-adv*adv_loss, fast_parameters_max, create_graph=True, allow_unused=True))
                    # print(grad_max)
                    # if l==0:
                    #     exit()
                    fast_parameters_max = []
...
def fairness_metrics(group_item_pred, group_item_rate):
    # other possible fairness metrics
    male_pred, female_pred = group_item_pred[0], group_item_pred[1]
...
def evaluate_test(args, model, dataloader):
    ...
    for c, batch in tqdm(enumerate(dataloader)):
        ...
        for i in range(x_spt.shape[0]):
            ...
            gender = int(x_qry[i][0, 4].detach().cpu().numpy())
            # N x 1
        ...
        for i in range(x_spt.shape[0]):
            ...
            for k in range(args.num_grad_steps_inner):
                ...
                for k, weight in enumerate(model.local_max_part.parameters()):
                    ...
                    fast_parameters_max.append(weight.fast)
            age_idx = x_spt[0][:, 4]
            location_idx = x_spt[0][:, 3]
            # number of interactions x dim
            user_emb = F.relu(model.fc_user(model.user_emb(location_idx, age_idx))).detach().cpu().numpy()
...
if __name__ == '__main__':
    ...
    train_data = Metabook(args, partition='train')
    if not args.fair:
        if not args.test:
            if args.adv:
