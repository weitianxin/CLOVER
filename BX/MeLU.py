...
class user_preference_estimator(torch.nn.Module):
    def __init__(self, config):
        ...
        if config.remove_age:
            self.fc_user_dim = config.embedding_dim * 1
        else:
            self.fc_user_dim = config.embedding_dim * 2
        self.fc_item_dim = config.embedding_dim * 3
        ...
        if config.loss==1:
            self.linear_out = Linear(self.fc2_out_dim, 10)
        else:
        ...
        if self.adv:
            if config.out==0:
                ...
                self.fc_user_out = Linear(self.fc2_in_dim//2, config.num_age)
                # if config.item_adv:
                ...
                if config.loss==0:
                    ...
                    self.fc_user_1 = Linear(20, self.fc2_in_dim//2)
                    self.fc_user_out = Linear(self.fc2_in_dim//2, config.num_age)
                    # self.fc_user_y_1 = Linear(1, self.fc2_in_dim//2)
            elif config.out==3:
                if config.loss==0:
                    self.fc_user_1 = Linear(6, self.fc2_in_dim)
                    self.fc_user_out = Linear(self.fc2_in_dim, config.num_age)
                elif config.loss==1:
                    raise ValueError("only for loss=0")
            elif config.out==4:
                if config.loss==0:
                    self.fc_user_1 = Linear(1, self.fc2_in_dim)
                    self.fc_user_out = Linear(self.fc2_in_dim, config.num_age)
                elif config.loss==1:
                    raise ValueError("only for loss=0")
            elif config.out==5:
            ...
            if self.adv2:
                if config.out2==0:
                    ...
                    self.fc_user_out_2 = Linear(self.fc2_in_dim//2, config.num_age)
                    # if config.item_adv:
                    #     self.fc_item_1 = Linear(self.fc2_in_dim//2, self.fc2_in_dim//2)
                    #     self.fc_item_out = Linear(self.fc2_in_dim//2, config.num_gender)
                    #     self.item_part = nn.Sequential(self.fc_item_1, nn.ReLU(), self.fc_item_out)
                    ...
                    if config.loss==0:
                        ...
                        self.fc_user_2 = Linear(20, self.fc2_in_dim//2)
                        self.fc_user_out_2 = Linear(self.fc2_in_dim//2, config.num_age)
                        # self.fc_user_y_1 = Linear(1, self.fc2_in_dim//2)
    ...
    def forward(self, x, y=None, training = True):
        ...
        location_idx = x[:,3]
        age_idx = x[:,4]
        item_emb = self.item_emb(author_idx, publisher_idx, year_idx)
        user_emb = self.user_emb(location_idx, age_idx)
        # for embed
        ...
        output = [x]
        con_age_idx = 1-age_idx
        con_user_emb = self.user_emb(location_idx, con_age_idx)
        ...
        if self.adv:
            ...
            if self.out==0:
                ...
                adv_loss = self.adv_loss(x_user, age_idx)
            # else:
            #     x_user = self.user_part(user_emb)
            #     con_gender_idx = 1-gender_idx
            #     con_user_emb = self.user_emb(con_gender_idx, age_idx, occupation_idx, area_idx)
            #     con_user_emb = F.relu(self.fc_user(con_user_emb))
            #     con_x_user = self.user_part(con_user_emb)
            #     adv_loss = torch.mean(self.cos_sim(x_user, con_x_user))
            elif self.out==1:
                # x_in = x_inter
                ...
                adv_loss = self.adv_loss(x_user, age_idx[0:1])
            elif self.out==2:
                if self.loss==0:
                    x_in = torch.cat([x, y],1)
                    # x_in = torch.mean(torch.cat([x, y],1),0,True)
                elif self.loss==1:
                    y_one_hot = F.one_hot(torch.squeeze(y.long()-1), num_classes=10)
            ...
            if self.adv2:
                if self.out2==0:
                    ...
                    # con_x_user = self.user_part(con_user_emb)
                    # gender_idx, x_user = torch.cat([gender_idx, con_gender_idx],0), torch.cat([x_user, con_x_user],0)
                    adv_loss += self.adv2/self.adv*self.adv_loss(x_user, age_idx)
                elif self.out2==1:
                    # x_in = x_inter
                    # x_in = torch.mean(x,0,True)
                    x_in = x
                    x_user = F.relu(self.fc_user_2(x_in))
                    # x_user = self.user_part_2(x_in)
                    if self.item_adv:
                        x_user += F.relu(self.fc_item_1(item_emb))
                    x_user = self.fc_user_out_2(x_user)
                    x_user = torch.mean(x_user,0,True)
                    adv_loss += self.adv2/self.adv*self.adv_loss(x_user, age_idx[0:1])
                elif self.out2==2:
                    if self.loss==0:
                        x_in = torch.cat([x, y],1)
                        # x_in = torch.mean(torch.cat([x, y],1),0,True)
